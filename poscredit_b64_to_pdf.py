# POSCREDIT BANK.B2POS LOGS TO DOCUMENTS CONVERTER
# v0.3.3
# ----
# Сделано с любовью и заботой cavemeat / wofiku
# Контакты: https://github.com/wofiku / ТГ: @cavemeat
# ----


# TODO: Реформат кода
# TODO: Массовая обработка шагов из листа (для МТС, ТБанк, к примеру)
# TODO: Drag'n'Drop по одному файлу (или скопом, смотря сколько файлов будет передано)
...
from os import getlogin, mkdir  # Создание директорий; будет создавать папки под доки по номеру заявки
...
banks_id: dict = {1: 'OTP', 2: 'RENCREDIT', 3: 'RUSFINANCE', 4: 'RSB', 5: 'VEXPRESS', 6: 'ALFABANK', 7: 'LETOBANK',
                  ...
                  21: 'SOVEST_LEADGEN', 22: 'CEBCARD', 23: 'INSUR_VSK', 24: 'INSUR_D2', 25: None, 26: 'HALVACARD',
                  ...
                  43: 'OTPCARD', 44: 'LOKOBANK', 45: 'MKBCARD', 47: 'ISLAM', 48: None, 49: 'PAYLATE', 51: 'DENUM',
                  52: 'SOVKOM_HALVA_MKK', 53: 'URALSIB', 54: 'SBER', 55: None, 56: 'DENUM_POS', 57: 'INSUR_TEKO'}
banks_full_rus: dict = {1: 'ОТП Банк', 2: 'Ренессанс Кредит Банк', 3: 'Русфинанс Банк', 4: 'Банк Русский Стандарт',
                        ...
                        18: 'Карта \"Халва\" Лидогенерация', 19: 'МФК СМСФинанс',
                        20: 'Кредит Европа Банк Лидогенерация', 21: 'Карта \"Совесть\" Лидогенерация',
                        22: 'Карта \"Кредит Европа Банк\"', 23: 'Страховое АО \"ВСК\"', 24: 'АО \"Д2 Страхование\"',
                        25: 'АО \"Группа Ренессанс Страхование\"', 26: 'Карта Рассрочки \"Халва\"', 27: 'Совкомбанк',
                        28: 'Запросто', 29: 'Альфа-Карта', 30: 'Экспобанк', 31: 'Хоум Кредит', 32: 'МТС Карта',
                        33: 'МКК \"Купи не копи\"', 35: 'Мокка', 36: 'Хоум Кредит ВД', 37: 'Хоум Экспресс',
                        39: 'Ванта Рассрочка', 40: 'Азия Кредит', 43: 'ОТП posline', 44: 'ЛОКО', 45: 'МКБ Карта',
                        47: '05RU', 48: 'МФК ОТП Финанс', 49: 'PayLate', 51: 'Денум', 52: 'СовкомБанк МКК',
                        53: 'Банк Уралсиб', 54: 'Сбербанк', 55: 'ООО МКК \"М-Финанс\"', 56: 'Денум плати частями',
                        57: 'ТЕКО'}

# Шаги в логах с документами, которые присылают банки 
banks_id_step_out_docfiles: dict = {1: 'out_CreateAgreement', 2: 'step4_out', 4: 'out_getPrintFormsRequest',
                                    6: 'stepGetPrintForms_out', 8: 'step4',
                                    9: 'step5_documents_out', 13: 'step2_out_',
                                    26: 'step3_out', 27: 'step3_out', 29: 'step11_out',
                                    44: 'stepapplication_status_out', 52: 'step3_out',
                                    53: 'callback_-uralsib-pos-loans-v1-applications-callback-attachments'}

# banks_id_step_out_docfiles[13] = 'step2_out_POSITIVE_DECISION_BANK_BRK'
# banks_id_step_out_docfiles[13] = 'step2_out_CREDIT_DOCUMENTATION_M'

# Объект документов в логе
obj_docs: dict = {1: 'Document', 2: 'documents', 4: 'printForm', 6: 'printForms', 8: 'return', 9: 'doc',
                  13: 'document', 26: 'documents', 27: 'documents', 29: 'printForms',
                  44: ('contractData', 'contractDocuments'), 52: 'documents', 53: ('attachmentsInfo', 'files')}
...
# Плашки для логгера
_prefix: str = f'[B64Conv {version}]'
super_secret_kit = ('Чипи чипи чапа чапа', 'Удаляем систему...', 'Лопата', '^‿^', f' {_prefix}'*9,
                    f'Привет, {getlogin()}', '')
logger_spl_file_read: str = _prefix + " Читаем файл \"{file_reading}\""
logger_spl_file_close: str = _prefix + " Выходим из файла \"{file_reading}\""
logger_spl_file_write: str = _prefix + " Успешно записано в \"{file_wrote}\""
logger_spl_surprisemeiambored: str = f"{_prefix} {random_choice(super_secret_kit)}"
...
# ФУНКЦИИ - ДОКУМЕНТЫ
# Проверка var на пустоту, замена на replace_with если пусто
def check_on_empty(var: any, replace_with: any) -> any:
    _empty: tuple = ('', ' ', 'null', 'Null', '<null>', '<Null>', None)

    return replace_with if var in _empty else var


# Проверяем, пуст ли документ; заменяем пустые поля, чтобы случайно не сломать файлы на выходе
...
def docs_handler_xml(content, bank_id):
    ...
    obj_form_docs = soup.find_all(obj_docs[bank_id])  # Находим все тэги с документами
    return obj_form_docs


def docs_handler_json(content, bank_id):
    full_json = json_loads(content)
    obj_docs_full_form = obj_docs[bank_id]
...
def arch_export_out_docs(archive: str) -> None:
    ...
    step_name = banks_id_step_out_docfiles[bank_id]
